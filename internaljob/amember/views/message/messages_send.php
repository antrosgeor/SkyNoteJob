...
<div class="row"><!-- Start div row -->
  ...
  <a class="list-group-item list-group-item-success"  href="?page=message/messages">  
    <?php 
      $sql = "SELECT * FROM messages WHERE user_from = '$userid'";
      $result=mysqli_query($dbc, $sql);
      $row = mysqli_num_rows($result);
    ?> <strong><i> Send Messages </i></strong><span class="badge"> <?php echo "$row" ?> </span>
  </a>
  <table class="table table-inbox table-hover">
    <tbody>
    <?php 
        $q = "SELECT * FROM messages WHERE user_from = '$userid' ORDER BY date DESC";
